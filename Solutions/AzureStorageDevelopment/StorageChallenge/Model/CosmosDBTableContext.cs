...
namespace CSSTDSolution.Models
{
    public class CosmosDBTableContext : ICosmosDBTableContext
    {
        ...
        {
            var endpoint = $"https://{accountName}.table.cosmosdb.azure.com"; var uri = new Uri(endpoint);
            client = new CloudTableClient(uri, new Microsoft.Azure.Cosmos.Table.StorageCredentials(accountName, key));
        }
        public void CreateTable(string tableName)
        {
            var table = client.GetTableReference(tableName); table.CreateIfNotExists();
        }
        ...
        {
            var table = client.GetTableReference(tableName);
            var query = new TableQuery<ProductMention>() { FilterString = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, $"{platform}:{product}"), };
    ...
    public class ProductMention : TableEntity, IProductMention
    {
        ...
        public string Product
        {
            ...
            set { this.product = value; setPartition(); }
        }
        public string Platform
        {
            ...
            set
            {
                this.platform = value; setPartition();
            }
        }
